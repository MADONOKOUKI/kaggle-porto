...
if __name__  == '__main__':
  ...
  logger.debug('data preparation end : {} '.format(use_cols.shape,use_cols))
  ...
  x_test = df[use_cols].sort_value('id')

  logger.info('test data load end {}'.format(x_test.shape))
  pred_test = clf.predict_proba(x_test)

  df_submit = pd.read_csv(SAMPLE_SUBMIT_FILE).sort_value('id')
